...
namespace IrysIntensity
{
    class BNXParser
    {
        private static Dictionary<String,String> runIds = new Dictionary<String, String>();

        //returns run name (without the word 'swap' in the beginning) and month
        private static Tuple<string, string> ParseRunData(string runDataLine)
        {
            string runName = "";
            string runMonth = "";
            ...
            return Tuple.Create(runName, runMonth);
        }
            ...
            using (var fileStream = File.OpenRead(bnx_file_path))
            {
                using (StreamReader streamReader = new StreamReader(fileStream))
                {
                    ...
                    while ((line = streamReader.ReadLine()) != null)
                    {
                        ...
                        {
                            Tuple<string, string> runData = ParseRunData(line);
                            //add run to database if the combination (projectId, run name, run month) doesn't exist
                            DatabaseManager.AddRun(projectId, runData.Item1, runData.Item2);
                        }
                        else if (line.StartsWith("0")) //all molecule information lines start with '0'
                        {
                            ...
                            {
                                DatabaseManager.AddMolecule(projectId, runId, int.Parse(bnxMolInfo[1]), int.Parse(bnxMolInfo[7]), int.Parse(bnxMolInfo[6]), float.Parse(bnxMolInfo[2]), 1,
                            ...
                            {
                                DatabaseManager.AddMolecule(projectId, runId, int.Parse(bnxMolInfo[1]), int.Parse(bnxMolInfo[7]), int.Parse(bnxMolInfo[6]), float.Parse(bnxMolInfo[2]), 0,
                                0, 0, 0, "null", 0, "null", 0);
                            }
